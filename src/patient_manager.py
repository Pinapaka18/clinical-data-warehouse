# src/patient_manager.py

import tkinter as tk
from tkinter import simpledialog, messagebox
import pandas as pd
import os
from utils import generate_unique_visit_id, is_valid_date_format_yyyy_mm_dd
...
class PatientManager:
    ...
    def load_patient_data(self):
        if os.path.exists(self.patient_file):
            self.df = pd.read_csv(self.patient_file)
        else:
    ...
    def save_data(self):
        self.df.to_csv(self.patient_file, index=False)
    ...
    def add_patient(self):
        ...
        self.load_patient_data()  # Ensure latest data
        pid = str(pid).strip()
        ...
        if not is_valid_date_format_yyyy_mm_dd(visit_time):
            messagebox.showerror("Error", "Invalid date format.")
            return

        visit_id = generate_unique_visit_id(self.df['Visit_ID'].tolist())
